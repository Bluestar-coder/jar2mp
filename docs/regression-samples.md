...
The script exits non-zero unless both modes report `BUILD SUCCESS`, `Failure type: NONE`, `exact_match=true`, content entries match, class-byte differences are `0`, nested-library differences are `0`, archive entry order matches, ZIP metadata differences are `0`, archive bytes match, and a rebuilt SHA-256 equals the original JAR SHA-256. It also enforces source coverage gates from each mode's `decompile-parity-report.md`: class parse failures must be `0`, and missing-source methods must be `0`. The summary records generated/reference Java file counts, current decompile-fallback counts, restoration score buckets (`overall`, `source`, `resource`, `runtime`, `verification`), remaining restoration gap count/category and non-penalizing observation count/category from `gap-summary.md`, explicit gate status for build verification, source coverage, byte package, and runtime observation, runtime launch support/status/event details when tracing is enabled, artifact fidelity detail columns, decompile parity risk counts, risk reason breakdowns, and `reference-only` / `generated-only` source file lists. Runtime event counts are de-duplicated by `kind/owner/target/value`; automatic smoke runs scope the trace agent to the entrypoint package plus `org.springframework.boot.loader.` to reduce large Spring Boot startup transformation cost, while manual agent use remains broad unless `jar2mp.traceIncludes` is set. The runtime observation gate reports `NOT_RUN` by default, `PASS_EXIT_ZERO` for a complete traced run, `PASS_HEALTHY_TIMEOUT` when a long-running app emits trace events and responds to the local HTTP startup probe before timeout, `WARN_STARTED_TIMEOUT` when events are collected without health evidence, `FAIL_STARTUP_FAILED_TIMEOUT` or `FAIL_STARTUP_FAILED_EXIT` when Spring Boot startup failure output appears before timeout or non-zero exit, or an explicit skip/failure status otherwise, so byte-level package equality is not hidden behind a runtime score of `0`; Redis/connection-refused external dependency startup failures are recorded as zero-impact `runtime_environment` observations and do not lower the byte-level restoration score or remaining restoration gap count, while other startup failure statuses are reported as the terminal runtime gap and suppress secondary missing-kind gaps for static file/socket/resource calls that the failed launch could not exercise. The parity risk columns include HIGH/MEDIUM/LOW methods, missing-source methods, reflection methods, invokedynamic methods, and methods that need user parameter or local-variable names but lack LocalVariableTable names; reflection risk is matched by `java/lang/Class`, `java/lang/reflect`, and known reflection utility owners instead of ordinary business method names such as `getMethod()` or `getField()`; invokedynamic details include bootstrap methods and arguments for lambda implementation targets or string-concat recipes; pure `StringConcatFactory` string-concat invokedynamic calls are still counted as invokedynamic facts, but are not MEDIUM risks; the LocalVariableTable missing-name count excludes synthetic enum switch-map support classes, bridge methods, enum support methods, lambda deserialization support methods, outer-this constructors, and monitor temporaries whose source-level semantics are already covered by surrounding source. Each `decompile-parity-report.md` also includes a `Risk method index` for HIGH/MEDIUM methods before the detailed class sections, with reasons such as `lambda metafactory invokedynamic`, `invokedynamic`, and `missing debug names`; the summary aggregates those reasons per mode. When build, source coverage, byte package, and source-recompiled class-byte gates pass, the decompile parity risk section remains advisory source-review evidence rather than a restoration-gap list. The source diff report marks whether each differing Java source has a corresponding `.class` entry in the original JAR (`root`, `BOOT-INF/classes`, or `WEB-INF/classes`), so reference-project local additions are not mistaken for jar2mp decompilation gaps. Shared Java content diffs are split into format-only, import-only, decompiler-artifact, byte-equivalent-text, and substantive buckets; byte-equivalent-text means the text still differs from the reference checkout while the generated source recompiles to the original class bytes, so substantive is reserved for same-path text differences without class-byte equivalence evidence. The resolved shared Java content-diff count is the sum of the four non-substantive evidence buckets, making the remaining same-path source repair surface visible at a glance. These non-gating parity counts are reported as progress evidence; final package byte equality is proven by the package-record and byte-exact artifact fidelity outputs, not by the runtime-observation portion of `restoration-score.md`.
