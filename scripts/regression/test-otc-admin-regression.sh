...
assert_contains "${script_source}" "shared_java_resolved_content_diff_files" "csv resolved shared java content diff column"
...
assert_contains "${script_source}" "shared_java_byte_equivalent_text_diff_files" "csv shared java byte-equivalent text diff column"
...
assert_contains "${script_source}" "Resolved shared Java content differences" "markdown resolved content diff count"
...
assert_contains "${script_source}" "Byte-equivalent textual shared Java content differences" "markdown byte-equivalent text diff count"
assert_contains "${script_source}" "Substantive shared Java content differences" "markdown substantive content diff count"
assert_contains "${script_source}" "byte_equivalent_text" "byte-equivalent text diff classification"
...
assert_contains "${script_source}" "package_record_source_rebuild_byte_gate" "csv package-record source rebuild byte gate column"
...
assert_contains "${script_source}" "byte_exact_source_rebuild_byte_gate" "csv byte-exact source rebuild byte gate column"
...
assert_contains "${script_source}" "Source rebuild bytes" "markdown source rebuild byte gate header"
...
assert_contains "${help_output}" "format-only, import-only, decompiler-artifact" "source content diff classification purpose"
assert_contains "${help_output}" "byte-equivalent-text, and substantive" "source content diff byte-equivalent classification purpose"
assert_contains "${help_output}" "original JAR class" "source diff class presence"
...
assert_contains "${help_output}" "remaining restoration gap / non-penalizing observation summaries" "remaining gap and observation summary purpose"
assert_contains "${help_output}" "gate status for build" "gate status purpose"
assert_contains "${help_output}" "source rebuild bytes" "source rebuild byte gate purpose"
